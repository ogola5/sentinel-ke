...
def train_graphsage(
    ...
    *,
    ...
    x = torch.tensor(dataset.feature_matrix, dtype=torch.float32)
    y = torch.tensor(dataset.labels, dtype=torch.float32).view(-1, 1)

    src_idx: List[int] = []
    dst_idx: List[int] = []

    for i, j, _ in dataset.edges:
        src_idx.append(i)
        dst_idx.append(j)
        src_idx.append(j)
        dst_idx.append(i)
    ...
    class GraphSAGELayer(nn.Module):
        ...
        def forward(self, feats: torch.Tensor) -> torch.Tensor:
            ...
            neigh.index_add_(0, edge_dst, feats[edge_src])

            deg = torch.zeros((n_nodes, 1), dtype=feats.dtype, device=feats.device)
            ones = torch.ones((edge_dst.shape[0], 1), dtype=feats.dtype, device=feats.device)
            deg.index_add_(0, edge_dst, ones)
