...
class CampaignRepository:
    ...
    def find_active_campaign(
        ...
        *,
        ...
        """
        Active = status=active AND last_seen within window.
        """
        cutoff = now - window
        return (
            self.db.query(Campaign)
    ...
    def upsert_entity(
        ...
        *,
        ...
        """
        Insert or update a campaign entity.

        Role semantics:
        - If incoming role == "unknown": keep existing role
        - Otherwise: overwrite existing role
        """

        stmt = (
            insert(CampaignEntity)
            .values(
                campaign_id=campaign_id,
                entity_key=entity_key,
                entity_type=entity_type,
            ...
            .on_conflict_do_update(
                ...
                set_={
                    "last_seen": seen_at,
                    "role": case(
                        (role == "unknown", CampaignEntity.role),
                        else_=role,
                    ),
                },
            )
    ...
    def recompute_and_store_stats(self, *, campaign: Campaign) -> None:
        """
        Deterministic stats derived from campaign_entity table.
        O(n) per call — acceptable for MVP.
        """
        cid = campaign.id
