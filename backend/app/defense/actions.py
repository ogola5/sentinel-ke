...
_ACTION_CATALOG: tuple[DefenseActionDefinition, ...] = (
    ...
    DefenseActionDefinition(
        ...
        description="Place a temporary hold on a suspected bank or mobile-money account pending analyst escalation.",
        delivery_mode="webhook",
        continuity_preserving=False,
        target_hint="Bank or mobile-money account identifier",
        category="identity",
    ),
    DefenseActionDefinition(
        key="hold_cashout",
        label="Hold cash-out",
        description="Prevent suspicious cash-out or agent settlement while the fraud chain is investigated.",
        delivery_mode="webhook",
        continuity_preserving=False,
        target_hint="Agent, account, or transaction reference",
        category="identity",
    ),
    DefenseActionDefinition(
        key="suspend_sim_change",
        label="Suspend SIM change",
        description="Block further SIM-swap or number-porting changes on a suspected subscriber until verification completes.",
        delivery_mode="webhook",
        continuity_preserving=True,
        target_hint="Phone or subscriber identifier",
        category="identity",
    ),
    DefenseActionDefinition(
