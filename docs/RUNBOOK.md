...
- `backend/requirements.txt` as aggregate full-dependency list (runtime + dev/test + ML)
- `backend/requirements-dev.txt` for development tools (`pytest`, `jupyterlab`, data science libs)
- `INSTALL_ALL_REQUIREMENTS=1` to force one-shot install from `backend/requirements.txt` during Docker build
...
- By default, Docker build keeps split install behavior for faster incremental rebuilds.
- Set `INSTALL_ALL_REQUIREMENTS=1` when you want one build to include the entire dependency stack from `requirements.txt`.
